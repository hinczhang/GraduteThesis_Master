...
namespace ARETT
{
	...
	public class DataAccessUWP : IDataAccess
	{
		...
		private void CheckForEyeData(object source, ElapsedEventArgs e)
		{
			...
			try {
				...
				if (currentSpatialCoordinateSystem == null || !EyesApiAvailable)
				{
					...
					//context.Post(__ =>
			  //      {
					//	EventCenter.Broadcast(EventType.ShowCamTransform, "[UWPDataAccess] No currentSpatialCoordinateSystem or Eyes API not available!");
					//}, null);
				...
				//context.Post(__ =>
				//{
				//	EventCenter.Broadcast(EventType.ShowCamTransform, "[UWPDataAccess] currentSpatialCoordinateSystem is not null");
				//}, null);
				...
				if (pointerPose != null)
				{
					...
					if (eyes != null)
					{
						...
						if (lastEyeDataTimestamp != targetTimeUnix) {
							...
							//context.Post(__ =>
							//{
							//	EventCenter.Broadcast(EventType.ShowCamTransform, "eyes.Gaze.HasValue");
							//}, null);
		...
		public void UnityUpdate()
		{
			...
			//currentSpatialCoordinateSystem = (SpatialCoordinateSystem)Marshal.GetObjectForIUnknown(WorldManager.GetNativeISpatialCoordinateSystemPtr());
			...
			//if (currentSpatialCoordinateSystem == null)
			//	{

			//		context.Post(__ =>
			//        {
			//			EventCenter.Broadcast(EventType.ShowCamTransform, "UnityUpdate: No currentSpatialCoordinateSystem");
			//		}, null);
